...
export default function SuperAdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "consultations" | "clients" | "users" | "kanban" | "chat">("overview");
  ...
  const [newUserRole, setNewUserRole] = useState<"client" | "manager" | "admin" | "superadmin">("client");
  const [newUserPlan, setNewUserPlan] = useState("Web Tradicional");
  
  // Edit User Inputs
  const [editUserName, setEditUserName] = useState("");
  const [editUserEmail, setEditUserEmail] = useState("");
  const [editUserCompany, setEditUserCompany] = useState("");
  const [editUserRole, setEditUserRole] = useState<"client" | "manager" | "admin" | "superadmin">("client");
  const [editUserPlan, setEditUserPlan] = useState("Web Tradicional");
  ...
  const fetchConsultations = async () => {
    ...
    try {
      const res = await fetch("/api/admin/consultations", { headers: getAdminHeaders() });
      const data = await res.json();
      if (res.ok && data.consultations) {
        setConsultationsList(data.consultations);
      }
    } catch {
      console.warn("Error fetching consultations");
    } finally {
      ...
      if (clientsData.clients?.length) {
        setClients(
          clientsData.clients.map((u: any) => ({
            ...
            projectStatus: u.projectStatus || "en_produccion",
            railwayStatus: u.railwayStatus || "activo",
            startDate: u.createdAt ? u.createdAt.slice(0, 10) : "2026-06-15",
            notes: u.projectDescription || "",
            deployedUrl: u.deployedUrl,
            thumbnailUrl: u.thumbnailUrl,
            techStack: typeof u.techStack === "string" ? u.techStack.split(", ") : u.techStack,
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok && data.success) {
        toast.success(`Usuario ${newUserName} creado correctamente como ${newUserRole}`);
  ...
  const openEditUserModal = (u: any) => {
    ...
    setEditUserPlan(u.plan || "Web Tradicional");
  ...
  const handleUpdateUser = async (e: React.FormEvent) => {
    ...
    setSavingUser(true);
    const userId = selectedUserForAction._id || selectedUserForAction.id;
    try {
      ...
      const data = await res.json();
      if (res.ok && data.success) {
        toast.success("Usuario y rol actualizados correctamente ✓");
  ...
  const handleDeleteUser = async (u: any) => {
    ...
    if (!window.confirm(`¿Estás seguro de eliminar al usuario "${u.name}" (${u.email})?`)) return;
    try {
      const res = await fetch(`/api/admin/users/${userId}`, {
        method: "DELETE",
        headers: getAdminHeaders(),
  ...
  const openUserPasswordModal = (u: any) => {
    setSelectedUserForAction(u);
    setNewPasswordValue("");
  ...
  const handleUpdateUserPassword = async (e: React.FormEvent) => {
    e.preventDefault();
    console.log("[PasswordUpdate:Admin] Form submission started", {
      selectedUser: selectedUserForAction,
      newPasswordLength: newPasswordValue?.length || 0,
    });

    if (!selectedUserForAction || !newPasswordValue) {
      console.warn("[PasswordUpdate:Admin] Missing required fields", { selectedUserForAction, newPasswordValue });
      return;
    }

    setSavingUser(true);
    const userId = selectedUserForAction._id || selectedUserForAction.id;
    const requestPayload = { currentPassword: "", newPassword: newPasswordValue };
    const requestUrl = `/api/users/${userId}/password`;

    console.log("[PasswordUpdate:Admin] Sending PUT request", { url: requestUrl, payload: requestPayload });
    ...
    try {
      ...
      const data = await res.json();
      console.log("[PasswordUpdate:Admin] Server response", { status: res.status, ok: res.ok, data });

      if (res.ok && data.success) {
        toast.success(`Contraseña de ${selectedUserForAction.name} actualizada correctamente ✓`);
        setShowUserPasswordModal(false);
        setNewPasswordValue("");
      } else {
        console.error("[PasswordUpdate:Admin] Error response from backend", data);
        toast.error(data.error || "Error actualizando contraseña.");
      }
    } catch (err) {
      console.error("[PasswordUpdate:Admin] Network or execution error", err);
      toast.error("Error de conexión al servidor.");
    } finally {
      setSavingUser(false);
    }
    ...
    <div className="bg-slate-950 text-slate-100 h-screen max-h-screen overflow-hidden flex flex-col font-sans">
      ...
      <div className="flex-1 flex flex-col md:flex-row min-h-0 overflow-hidden">
        ...
        >
          ...
          {[
            ...
            { id: "consultations", label: "Consultas 15 Min", icon: Calendar },
            { id: "clients", label: "Clientes & Suscripciones", icon: Users },
            { id: "users", label: "Gestión de Usuarios", icon: ShieldCheck },
        ...
        <main className="flex-1 p-3 sm:p-4 md:p-5 overflow-y-auto min-h-0 custom-scrollbar">
          ...
          {/* TAB: 15-MIN FREE CONSULTATIONS */}
          {activeTab === "consultations" && (
            <div className="space-y-4">
              <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-3">
                <div>
                  <h2 className="text-xl sm:text-2xl font-black text-white tracking-tight flex items-center gap-2">
                    <span>⚡ Consultas Gratuitas de 15 Minutos</span>
                    <span className="text-xs font-bold px-2.5 py-0.5 rounded-full bg-blue-500/20 text-blue-400 border border-blue-500/30">
                      {consultationsList.length} Registradas
                    </span>
                  </h2>
                  <p className="text-slate-400 text-xs font-medium">
                    Todas las citas agendadas desde la web, chatbot y landing pages. Notificaciones automáticas enviadas a <span className="text-white font-bold">yerctech@gmail.com</span>.
                  </p>
                </div>
                <button
                  onClick={fetchConsultations}
                  className="bg-slate-800 hover:bg-slate-700 text-white font-bold px-3 py-2 rounded-xl text-xs flex items-center gap-1.5 cursor-pointer border border-slate-700/60"
                >
                  <RefreshCw className={`w-3.5 h-3.5 ${consultationsLoading ? "animate-spin" : ""}`} /> Actualizar Citas
                </button>
              </div>
              ...
              {consultationsLoading ? (
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-12 text-center space-y-3">
                  ...
                  <h3 className="text-base font-bold text-white">No hay consultas agendadas todavía</h3>
                  <p className="text-xs text-slate-400 max-w-md mx-auto">
                    Cuando los prospectos completen el formulario de 15 minutos en la web o en el chatbot de IA, aparecerán listadas aquí inmediatamente.
                  </p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3.5">
                  {consultationsList.map((cons: any) => {
                    const statusColor = cons.status === "confirmada" ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/30" : cons.status === "completada" ? "bg-blue-500/10 text-blue-400 border-blue-500/30" : "bg-amber-500/10 text-amber-400 border-amber-500/30";
                    return (
                      <div key={cons._id || cons.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-4 space-y-3 flex flex-col justify-between hover:border-slate-700 transition-all shadow-md">
                        <div className="space-y-2">
                          <div className="flex items-center justify-between gap-2">
                            <span className="text-[11px] font-black text-white truncate">{cons.name}</span>
                            <span className={`text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full border ${statusColor}`}>
                              {cons.status || "pendiente"}
                            </span>
                          </div>

                          <div className="p-2.5 rounded-xl bg-slate-950/80 border border-slate-800/80 space-y-1 text-xs">
                            <div className="flex items-center justify-between text-slate-300">
                              <span className="text-[10px] text-slate-400 flex items-center gap-1 font-medium"><Calendar className="w-3 h-3 text-blue-400" /> Fecha:</span>
                              <span className="font-bold text-blue-400">{cons.date}</span>
                            </div>
                            <div className="flex items-center justify-between text-slate-300">
                              <span className="text-[10px] text-slate-400 flex items-center gap-1 font-medium"><Clock className="w-3 h-3 text-emerald-400" /> Horario:</span>
                              <span className="font-bold text-emerald-400">{cons.timeSlot} (PET)</span>
                            </div>
                            {cons.company && (
                              <div className="flex items-center justify-between text-slate-300">
                                <span className="text-[10px] text-slate-400 font-medium">Empresa:</span>
                                <span className="font-medium text-slate-200 truncate max-w-[150px]">{cons.company}</span>
                              </div>
                            )}
                          </div>
                          ...
                          <div className="space-y-1 text-[11px] text-slate-300">
                            <p className="flex items-center gap-1.5 truncate">
                              ...
                              <a href={`mailto:${cons.email}`} className="hover:underline text-blue-400 truncate">{cons.email}</a>
                            </p>
                            <p className="flex items-center gap-1.5">
                              <Phone className="w-3 h-3 text-slate-500 shrink-0" />
                              <span className="text-slate-200 font-mono">{cons.phone}</span>
                            </p>
                            {cons.notes && (
                              <p className="text-[10px] text-slate-400 italic bg-slate-950 p-2 rounded-lg border border-slate-800/50 mt-1 line-clamp-2">
                                "{cons.notes}"
                              </p>
                            )}
                          </div>
                        ...
                        <div className="pt-2 border-t border-slate-800/80 flex items-center justify-between gap-2">
                          <a
                            href={`https://wa.me/${cons.phone.replace(/\D/g, '')}?text=${encodeURIComponent(`Hola ${cons.name}, te saluda Yosward de Chamba Digital sobre tu consulta agendada para el ${cons.date} a las ${cons.timeSlot}.`)}`}
                            target="_blank"
                            rel="noreferrer"
                            className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-1.5 px-3 rounded-lg text-xs flex items-center gap-1.5 transition-colors shadow-sm cursor-pointer"
                          >
                            <MessageSquare className="w-3.5 h-3.5" /> WhatsApp
                          </a>
                          <select
                            value={cons.status || "pendiente"}
                            onChange={async (e) => {
                              const newStatus = e.target.value;
                              try {
                                await fetch(`/api/admin/consultations/${cons._id || cons.id}`, {
                                  method: "PUT",
                                  headers: getAdminHeaders(),
                                  body: JSON.stringify({ status: newStatus })
                                });
                            ...
                            className="bg-slate-950 border border-slate-700 text-slate-300 text-[10px] rounded-lg px-2 py-1 outline-none cursor-pointer"
                          >
          ...
          {/* TAB: USER MANAGEMENT (ADMINISTRADOR DE USUARIOS & ROLES) */}
          {activeTab === "users" && (
            <div className="space-y-4">
              <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-3">
                <div>
                  <h2 className="text-xl sm:text-2xl font-black text-white tracking-tight">Administrador de Usuarios & Roles</h2>
                  <p className="text-slate-400 text-xs font-medium">Gestión integral de credenciales, roles (SuperAdmin, Admin, Manager, Cliente), Passkeys y 2FA.</p>
                </div>
                <button
                  onClick={() => {
                    setNewUserName("");
                  ...
                  className="bg-blue-600 hover:bg-blue-500 text-white font-black px-4 py-2.5 rounded-xl text-xs uppercase tracking-wider flex items-center justify-center gap-2 transition-all cursor-pointer shadow-lg shadow-blue-600/20"
                >
              ...
              {/* Filters Bar */}
              <div className="bg-slate-900 border border-slate-800 p-3 rounded-2xl flex flex-col md:flex-row gap-3 items-center justify-between">
                <div className="relative flex-1 w-full">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950 border border-slate-800 focus:border-blue-500 rounded-xl pl-9 pr-4 py-2 text-xs text-white outline-none transition-colors"
                  />
                </div>
                <div className="flex items-center gap-2 w-full md:w-auto">
                  ...
                  <select
                    ...
                    className="bg-slate-950 border border-slate-800 text-slate-300 text-xs rounded-xl px-3 py-2 outline-none cursor-pointer w-full md:w-auto"
                  >
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-xs">
                    <thead className="bg-slate-950/80 text-slate-400 font-extrabold uppercase border-b border-slate-800">
                      <tr>
                        <th className="p-3.5">Usuario</th>
                        <th className="p-3.5">Empresa</th>
                        <th className="p-3.5">Rol</th>
                        <th className="p-3.5">Seguridad 2FA / Passkey</th>
                    ...
                    <tbody className="divide-y divide-slate-800 text-slate-200">
                      {usersList
                        .filter((u) => {
                          const matchesQuery = u.name?.toLowerCase().includes(userSearchQuery.toLowerCase()) || u.email?.toLowerCase().includes(userSearchQuery.toLowerCase()) || u.company?.toLowerCase().includes(userSearchQuery.toLowerCase());
                        ...
                        .map((u) => {
                          ...
                          return (
                            <tr key={u._id || u.id || u.email} className="hover:bg-slate-800/40 transition-colors">
                              <td className="p-3.5">
                              ...
                              <td className="p-3.5">
                                <div className="font-semibold text-slate-300">{u.company || "Chamba Digital"}</div>
                                <div className="text-[10px] text-slate-500">{u.plan || "Web Tradicional"}</div>
                              </td>
                              <td className="p-3.5">
                                <span className={`px-2.5 py-1 rounded-full border text-[10px] font-black uppercase tracking-wider ${roleBadgeColor}`}>
                                  {u.role || "client"}
                                </span>
                              </td>
                              <td className="p-3.5">
                                <div className="flex items-center gap-2">
                                  {u.twoFactorEnabled ? (
                                    <span className="px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-400 text-[10px] font-bold flex items-center gap-1 border border-emerald-500/20">
                                      <ShieldCheck className="w-3 h-3" /> 2FA Activo
                                    </span>
                                  ) : (
                                  ...
                                  {u.passkeys && u.passkeys.length > 0 ? (
                                    <span className="px-2 py-0.5 rounded bg-blue-500/10 text-blue-400 text-[10px] font-bold flex items-center gap-1 border border-blue-500/20">
                                      <Zap className="w-3 h-3" /> {u.passkeys.length} Passkey(s)
                                    </span>
                                  ) : null}
                                </div>
                              </td>
        ...
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl max-w-md w-full space-y-4">
            <h3 className="text-lg font-black text-white flex items-center gap-2">
              <Plus className="w-5 h-5 text-blue-400" /> Crear Nuevo Usuario
            </h3>
            <form onSubmit={handleCreateUser} className="space-y-3">
              ...
              <div>
                <label className="text-[11px] font-bold text-slate-400 uppercase">Correo Electrónico</label>
                <input
                  type="email"
                  required
                  value={newUserEmail}
              ...
              <div>
                <label className="text-[11px] font-bold text-slate-400 uppercase">Contraseña Inicial</label>
                <input
                  type="password"
                  required
                  minLength={6}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[11px] font-bold text-slate-400 uppercase">Empresa / Proyecto</label>
                  <input
                    type="text"
                    value={newUserCompany}
                    onChange={(e) => setNewUserCompany(e.target.value)}
                    placeholder="Ej. Surf School"
                    className="w-full bg-slate-950 border border-slate-800 focus:border-blue-500 rounded-xl px-3.5 py-2.5 text-xs text-white outline-none mt-1"
                  />
                </div>
                <div>
                  <label className="text-[11px] font-bold text-slate-400 uppercase">Rol de Usuario</label>
                  <select
                    value={newUserRole}
                    onChange={(e: any) => setNewUserRole(e.target.value)}
                    className="w-full bg-slate-950 border border-slate-800 focus:border-blue-500 rounded-xl px-3.5 py-2.5 text-xs text-white outline-none mt-1 cursor-pointer"
                  >
                    <option value="client">Cliente</option>
                    <option value="manager">Manager</option>
                    <option value="admin">Admin</option>
              ...
              <div>
                <label className="text-[11px] font-bold text-slate-400 uppercase">Plan Suscrito</label>
                <input
                  type="text"
                  value={newUserPlan}
                  onChange={(e) => setNewUserPlan(e.target.value)}
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3.5 py-2.5 text-xs text-white outline-none mt-1"
                />
              </div>
              <div className="flex justify-end gap-3 pt-2">
                <button
                  ...
                  className="text-slate-400 hover:text-white text-xs font-bold px-4 py-2 cursor-pointer"
                >
                  Cancelar
                </button>
                <button
                  type="submit"
                  disabled={savingUser}
                  className="bg-blue-600 hover:bg-blue-500 text-white font-bold px-5 py-2 rounded-xl text-xs uppercase cursor-pointer disabled:opacity-50"
                >
      ...
      {showEditUserModal && selectedUserForAction && (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl max-w-md w-full space-y-4">
            <h3 className="text-lg font-black text-white flex items-center gap-2">
              <Edit3 className="w-5 h-5 text-blue-400" /> Editar Usuario & Rol
            </h3>
            <form onSubmit={handleUpdateUser} className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-[11px] font-bold text-slate-400 uppercase">Rol</label>
                  <select
                    value={editUserRole}
                    onChange={(e: any) => setEditUserRole(e.target.value)}
                    className="w-full bg-slate-950 border border-slate-800 focus:border-blue-500 rounded-xl px-3.5 py-2.5 text-xs text-white outline-none mt-1 cursor-pointer font-bold"
                  >
                    <option value="client">Cliente</option>
                    <option value="manager">Manager</option>
                    <option value="admin">Admin</option>
              ...
              <div>
                <label className="text-[11px] font-bold text-slate-400 uppercase">Plan Contratado</label>
                <input
                  type="text"
                  value={editUserPlan}
      ...
      {showUserPasswordModal && selectedUserForAction && (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl max-w-md w-full space-y-4">
            <h3 className="text-lg font-black text-white flex items-center gap-2">
              <Lock className="w-5 h-5 text-amber-400" /> Cambiar Contraseña
            </h3>
            <p className="text-xs text-slate-400">
              Actualizar la clave de acceso para <strong className="text-white">{selectedUserForAction.name}</strong> ({selectedUserForAction.email}).
            </p>
            <form onSubmit={handleUpdateUserPassword} className="space-y-3">
              {/* Hidden input for password manager accessibility */}
              <input type="text" name="username" value={selectedUserForAction.email} readOnly className="hidden" autoComplete="username" />
              <div>
                <label className="text-[11px] font-bold text-slate-400 uppercase">Nueva Contraseña</label>
                <input
                  type="password"
                  required
                  minLength={6}
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button
                  type="button"
                  onClick={() => setShowUserPasswordModal(false)}
                  className="text-slate-400 hover:text-white text-xs font-bold px-4 py-2 cursor-pointer"
                >
                  Cancelar
                </button>
                <button
                  type="submit"
                  disabled={savingUser}
                  className="bg-amber-600 hover:bg-amber-500 text-white font-bold px-5 py-2 rounded-xl text-xs uppercase cursor-pointer disabled:opacity-50"
                >
